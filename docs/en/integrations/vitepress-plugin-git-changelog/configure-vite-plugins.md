...
We migrated the `locales` configurations to [UI config](/en/integrations/vitepress-plugin-git-changelog/configure-ui#locales-options). You no longer need to set `locales` for `GitChangelogMarkdownSection` plugin.

For information, please refer to [Migrate from v1 to v2](/en/releases/migrations/v1-to-v2).
